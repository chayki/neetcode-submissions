public class Solution {
    ...
    private bool[] acuteDiagOccupancy;
    private bool[] obtuseDiagOccupancy;
    ...
    public List<List<string>> SolveNQueens(int n) {
       ...
       // obtuse family diagonals: r-c
        ...
        // Pro tip: r+c spans from 0 to 2*n-2
        // r-c spans from -(n-1) to n-1 (add n-1 to make it 0 indexed) = 0 to 2n-2
        // with this indexing adjustments I can use arrays which are much faster due to cache locality than dictionaries

        var result = new List<List<string>>();
        columnOccupancy = new bool[n];
       rowFormation = new char[n][];
       acuteDiagOccupancy = new bool[2*n-1];
       obtuseDiagOccupancy = new bool[2*n-1];
    ...
    {
        return !acuteDiagOccupancy[row+col] && !obtuseDiagOccupancy[row-col+n-1] && !columnOccupancy[col];
    }

    private void OccupyPosition(int row, int col, int n)
    {
        acuteDiagOccupancy[row+col] = true;
        obtuseDiagOccupancy[row-col+n-1] = true;
    ...
    private void UnOccupyPosition(int row, int col, int n)
    {
        ...
        obtuseDiagOccupancy[row-col+n-1] = false;
